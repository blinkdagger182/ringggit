...
struct InsightsView: View {
    ...
    @AppStorage("chartTimeFrame", store: UserDefaults(suiteName: "group.com.riskcreatives.duit")) var chartType = 1
    @AppStorage("insightsViewIncomeFiltering", store: UserDefaults(suiteName: "group.com.riskcreatives.duit")) var insightsIncome: Bool = true
...
//                    .font(Font.satoshi(18, weight: .medium))
    ...
    func hasTransactions(inLastDays days: Int) -> Bool {
        ...
        let startDate = calendar.date(byAdding: .day, value: -(days - 1), to: Date.now) ?? Date.now

        return transactions.contains { transaction in
            transaction.wrappedDate >= startDate
        }
    }

    func syncIncomePreference() {
        ...
        if chartType == 1 {
            let start = calendar.date(byAdding: .day, value: -29, to: Date.now) ?? Date.now
            startDate = calendar.startOfDay(for: start)
            type = 4
        } else if chartType == 2 {
            let components = calendar.dateComponents([.year, .month], from: Date.now)
            let currentMonth = calendar.date(from: components) ?? Date.now
            startDate = calendar.date(byAdding: .month, value: -5, to: currentMonth) ?? Date.now
            type = 5
        } else if chartType == 3 {
            let components = calendar.dateComponents([.year, .month], from: Date.now)
            let currentMonth = calendar.date(from: components) ?? Date.now
            startDate = calendar.date(byAdding: .month, value: -11, to: currentMonth) ?? Date.now
            type = 6
        } else if chartType == 4 {
            startDate = getStartOfMonth(startDay: UserDefaults(suiteName: "group.com.riskcreatives.duit")?.integer(forKey: "firstDayOfMonth") ?? 1)
            type = 2
        } else {
            let yearComponents = calendar.dateComponents([.year], from: Date.now)
            startDate = calendar.date(from: yearComponents) ?? Date.now
            type = 3
        }

        let incomeTotal = dataController.getInsights(type: type, date: startDate, income: true).amount
        let expenseTotal = dataController.getInsights(type: type, date: startDate, income: false).amount

        if insightsIncome && incomeTotal == 0 && expenseTotal > 0 {
            insightsIncome = false
        }
    }
}
...
struct HorizontalPieChartView: View {
    ...
    init(date: Date, categoryFilter: Binding<Category?>?, categoryFilterMode: Binding<Bool>, selectedDate: Binding<Date?>?, chosenAmount: Binding<Double>, chosenName: Binding<String>, type: ChartTimeFrame, income: Bool) {
        ...
        case .lastMonth, .lastSixMonths, .lastTwelveMonths:
            endPredicate = NSPredicate(format: "%K < %@", #keyPath(Transaction.date), Date.now as CVarArg)
        case .thisMonth:
...
struct SingleGraphView: View {
    ...
    @AppStorage("incomeTracking", store: UserDefaults(suiteName: "group.com.riskcreatives.duit")) var incomeTracking: Bool = true
    ...
    @AppStorage("firstDayOfMonth", store: UserDefaults(suiteName: "group.com.riskcreatives.duit")) var firstDayOfMonth: Int = 1
    ...
    var dateString: String {
        ...
        if type == 1 {
            ...
            dateFormatter.dateFormat = "d MMM"
            let endDate = Calendar.current.date(byAdding: .day, value: 29, to: date) ?? Date.now
            return dateFormatter.string(from: date) + " - " + dateFormatter.string(from: min(endDate, Date.now))
...
//                            .font(Font.satoshi(14, weight: .semibold))
            ...
            if incomeFiltering {
                if type == 1 {
                    ...
                    SingleRollingDaysBarGraphView(startDate: date, date: $selectedDate, mode: $categoryFilterMode, amount: $selectedDateAmount, dataController: dataController, income: income)
                } else if type == 5 {
                    SingleRollingMonthsBarGraphView(startDate: date, date: $selectedDate, mode: $categoryFilterMode, amount: $selectedDateAmount, dataController: dataController, income: income, months: 6)
                } else if type == 6 {
                    SingleRollingMonthsBarGraphView(startDate: date, date: $selectedDate, mode: $categoryFilterMode, amount: $selectedDateAmount, dataController: dataController, income: income, months: 12)
                }
            }
        }
    }
    ...
    init(showingDate: Date, date: Binding<Date?>?, mode: Binding<Bool>, categoryName: String, categoryAmount: Double, currencySymbol: String, showCents: Bool, dataController: DataController, income: Binding<Bool>, incomeFiltering: Binding<Bool>, type: Int) {
        ...
        if type == 1 {
            ...
            lastDate = Calendar.current.date(byAdding: .day, value: -30, to: showingDate) ?? Date.now
        } else if type == 5 {
            lastDate = Calendar.current.date(byAdding: .month, value: -6, to: showingDate) ?? Date.now
        } else {
            lastDate = Calendar.current.date(byAdding: .month, value: -12, to: showingDate) ?? Date.now
        }
...
//        return abs(offset) > UIScreen.main.bounds.width * 0.3
    ...
    @AppStorage("insightsViewIncomeFiltering", store: UserDefaults(suiteName: "group.com.riskcreatives.duit")) var income: Bool = true
    @AppStorage("incomeTracking", store: UserDefaults(suiteName: "group.com.riskcreatives.duit")) var incomeTracking: Bool = true
...
struct LastThirtyDaysGraphView: View {
    @EnvironmentObject var dataController: DataController
    private var didSave = NotificationCenter.default.publisher(for: .NSManagedObjectContextDidSave)
    @State private var refreshID = UUID()
    @State private var refreshID1 = UUID()
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: 18) {
                SingleGraphView(showingDate: startDate, date: $selectedDate, mode: $categoryFilterMode, categoryName: chosenCategoryName, categoryAmount: chosenCategoryAmount, currencySymbol: currencySymbol, showCents: showCents, dataController: dataController, income: $income, incomeFiltering: $incomeFiltering, type: 4)
                    .drawingGroup()
                    .id(refreshID)
                    .padding(.horizontal, 30)
                ...
                Group {
                    if !incomeFiltering {
                        FilteredInsightsView(startDate: startDate, type: 4)
                            .padding(.bottom, 70)
                            .padding(.horizontal, 20)
                    } else {
                        if selectedDate == nil {
                            HorizontalPieChartView(date: startDate, categoryFilter: $categoryFilter, categoryFilterMode: $categoryFilterMode, selectedDate: $selectedDate, chosenAmount: $chosenCategoryAmount, chosenName: $chosenCategoryName, type: .lastMonth, income: income)
                                .padding(.horizontal, 30)
                                .padding(.bottom, 70)
                                .id(refreshID1)

                            if categoryFilterMode {
                                FilteredCategoryInsightsView(category: categoryFilter, date: startDate, type: .lastMonth)
                                    .padding(.bottom, 70)
                                    .padding(.horizontal, 20)
                            }
                        } else {
...
struct LastSixMonthsGraphView: View {
    @EnvironmentObject var dataController: DataController
    private var didSave = NotificationCenter.default.publisher(for: .NSManagedObjectContextDidSave)
    @State private var refreshID = UUID()
    @State private var refreshID1 = UUID()
    ...
    var startDate: Date {
        ...
        let components = calendar.dateComponents([.year, .month], from: Date.now)
        let currentMonth = calendar.date(from: components) ?? Date.now
        return calendar.date(byAdding: .month, value: -5, to: currentMonth) ?? Date.now
    }

    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: 18) {
                SingleGraphView(showingDate: startDate, date: $selectedDate, mode: $categoryFilterMode, categoryName: chosenCategoryName, categoryAmount: chosenCategoryAmount, currencySymbol: currencySymbol, showCents: showCents, dataController: dataController, income: $income, incomeFiltering: $incomeFiltering, type: 5)
                    .drawingGroup()
                    .id(refreshID)
                    .padding(.horizontal, 30)
                    .padding(.bottom, incomeFiltering ? 5 : 20)
                ...
                Group {
                    if !incomeFiltering {
                        FilteredInsightsView(startDate: startDate, type: 5)
                            .padding(.bottom, 70)
                            .padding(.horizontal, 20)
                    } else {
                        if selectedDate == nil {
                            HorizontalPieChartView(date: startDate, categoryFilter: $categoryFilter, categoryFilterMode: $categoryFilterMode, selectedDate: $selectedDate, chosenAmount: $chosenCategoryAmount, chosenName: $chosenCategoryName, type: .lastSixMonths, income: income)
                                .padding(.horizontal, 30)
                                .padding(.bottom, 70)
                                .id(refreshID1)

                            if categoryFilterMode {
                                FilteredCategoryInsightsView(category: categoryFilter, date: startDate, type: .lastSixMonths)
                                    .padding(.bottom, 70)
                                    .padding(.horizontal, 20)
                            }
                        } else {
...
struct LastTwelveMonthsGraphView: View {
    @EnvironmentObject var dataController: DataController
    private var didSave = NotificationCenter.default.publisher(for: .NSManagedObjectContextDidSave)
    @State private var refreshID = UUID()
    @State private var refreshID1 = UUID()
    ...
    var startDate: Date {
        ...
        let components = calendar.dateComponents([.year, .month], from: Date.now)
        let currentMonth = calendar.date(from: components) ?? Date.now
        return calendar.date(byAdding: .month, value: -11, to: currentMonth) ?? Date.now
    }

    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: 18) {
                SingleGraphView(showingDate: startDate, date: $selectedDate, mode: $categoryFilterMode, categoryName: chosenCategoryName, categoryAmount: chosenCategoryAmount, currencySymbol: currencySymbol, showCents: showCents, dataController: dataController, income: $income, incomeFiltering: $incomeFiltering, type: 6)
                    .drawingGroup()
                    .id(refreshID)
                    .padding(.horizontal, 30)
                    .padding(.bottom, incomeFiltering ? 5 : 20)
                ...
                Group {
                    if !incomeFiltering {
                        FilteredInsightsView(startDate: startDate, type: 6)
                            .padding(.bottom, 70)
                            .padding(.horizontal, 20)
                    } else {
                        if selectedDate == nil {
                            HorizontalPieChartView(date: startDate, categoryFilter: $categoryFilter, categoryFilterMode: $categoryFilterMode, selectedDate: $selectedDate, chosenAmount: $chosenCategoryAmount, chosenName: $chosenCategoryName, type: .lastTwelveMonths, income: income)
                                .padding(.horizontal, 30)
                                .padding(.bottom, 70)
                                .id(refreshID1)

                            if categoryFilterMode {
                                FilteredCategoryInsightsView(category: categoryFilter, date: startDate, type: .lastTwelveMonths)
                                    .padding(.bottom, 70)
                                    .padding(.horizontal, 20)
                            }
                        } else {
            ...
            VStack(spacing: 18) {
                ...
                Group {
                    if !incomeFiltering {
                        ...
                        if selectedDate == nil {
                            HorizontalPieChartView(date: showingMonth, categoryFilter: $categoryFilter, categoryFilterMode: $categoryFilterMode, selectedDate: $selectedDate, chosenAmount: $chosenCategoryAmount, chosenName: $chosenCategoryName, type: .thisMonth, income: income)
                                .padding(.horizontal, 30)
                                .padding(.bottom, 70)
                                .id(refreshID1)

                            if categoryFilterMode {
                                FilteredCategoryInsightsView(category: categoryFilter, date: showingMonth, type: .thisMonth)
                                    .padding(.bottom, 70)
                                    .padding(.horizontal, 20)
                            }
                        } else {
...
struct SingleRollingDaysBarGraphView: View {
    @Binding var selectedDate: Date?
    @Binding var categoryFilterMode: Bool
    @Binding var selectedDateAmount: Double
    ...
    let numberArray = [1, 6, 11, 16, 21, 26]
    ...
    var body: some View {
        ZStack(alignment: .top) {
                ...
                HStack(alignment: .top, spacing: 2) {
                    ForEach(days, id: \.self) { day in
                        ZStack(alignment: .bottom) {
                            ...
                            AnimatedBarGraph(index: days.firstIndex(of: day) ?? 0)
                                .frame(height: getBarHeight(point: dayDictionary[day] ?? 0, maxi: getMax))
                                .opacity(selectedDate == nil ? 1 : (selectedDate == day ? 1 : 0.4))
                                .zIndex(0)
                                .overlay(alignment: .bottom) {
            ...
            AverageLineView(getMax: getMax, average: average)
                .opacity(actualDays <= 1 ? 0 : 1)
        }
        .onChange(of: selectedDate) { _ in
            selectedDateAmount = dayDictionary[selectedDate ?? Date.now] ?? 0.0
    ...
    init(startDate: Date, date: Binding<Date?>?, mode: Binding<Bool>, amount: Binding<Double>, dataController: DataController, income: Bool) {
        _selectedDate = date ?? Binding.constant(nil)
        _categoryFilterMode = mode
        _selectedDateAmount = amount

        let loaded = dataController.getInsights(type: 4, date: startDate, income: income)
...
struct SingleRollingMonthsBarGraphView: View {
    @Binding var selectedDate: Date?
    @Binding var categoryFilterMode: Bool
    @Binding var selectedDateAmount: Double
                ...
                HStack(alignment: .top, spacing: 4) {
                    ForEach(months, id: \.self) { month in
                        ZStack(alignment: .bottom) {
                            ...
                            AnimatedBarGraph(index: months.firstIndex(of: month) ?? 0)
    ...
    init(startDate: Date, date: Binding<Date?>?, mode: Binding<Bool>, amount: Binding<Double>, dataController: DataController, income: Bool, months count: Int) {
        _selectedDate = date ?? Binding.constant(nil)
        _categoryFilterMode = mode
        _selectedDateAmount = amount
        monthsCount = count

        let type = count == 6 ? 5 : 6
        let loaded = dataController.getInsights(type: type, date: startDate, income: income)
            ...
            VStack(spacing: 18) {
                ...
                Group {
                    if !incomeFiltering {
                        ...
                        if selectedDate == nil {
                            HorizontalPieChartView(date: showingYear, categoryFilter: $categoryFilter, categoryFilterMode: $categoryFilterMode, selectedDate: $selectedDate, chosenAmount: $chosenCategoryAmount, chosenName: $chosenCategoryName, type: .thisYear, income: income)
                                .padding(.horizontal, 30)
                                .padding(.bottom, 70)
                                .id(refreshID1)

                            if categoryFilterMode {
                                FilteredCategoryInsightsView(category: categoryFilter, date: showingYear, type: .thisYear)
                                    .padding(.bottom, 70)
                                    .padding(.horizontal, 20)
                            }
                        } else {
...
struct ChartTimePickerView: View {
    @Namespace var animation
    @State var timeframe = ChartTimeFrame.lastMonth
    @Binding var showMenu: Bool

    @AppStorage("colourScheme", store: UserDefaults(suiteName: "group.com.riskcreatives.duit")) var colourScheme: Int = 0
    ...
    var body: some View {
        ...
        .onChange(of: timeframe) { _ in
            if timeframe == .lastMonth {
                ...
                chartType = 2
            } else if timeframe == .lastTwelveMonths {
                chartType = 3
        ...
        .onAppear {
            if chartType == 1 {
                timeframe = ChartTimeFrame.lastMonth
            } else if chartType == 2 {
                timeframe = ChartTimeFrame.lastSixMonths
            } else if chartType == 3 {
                timeframe = ChartTimeFrame.lastTwelveMonths
            } else if chartType == 4 {
                timeframe = ChartTimeFrame.thisMonth
            } else if chartType == 5 {
                timeframe = ChartTimeFrame.thisYear
            }
        }
    }
}
